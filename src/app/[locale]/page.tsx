  ...
  const features = [
    {
      ...
      desc: locale === "ka" ? "მუდმივი მხარდაჭერა საქართველოში" : "Full support network in Georgia",
    },
    {
      icon: "🛡️",
      title: locale === "ka" ? "ოფიციალური გარანტია" : "Official Warranty",
      desc: locale === "ka" ? "BYD-ის ავტორიზებული სერვის" : "BYD authorized service center",
    },
    {
      icon: "🔋",
      title: locale === "ka" ? "Blade ბატარეა" : "Blade Battery",
      desc: locale === "ka" ? "მსოფლიოში ყველაზე უსაფრთხო" : "World safest EV battery tech",
    },
    {
      icon: "🌍",
      title: locale === "ka" ? "გლობ. ლიდერი" : "Global Leader",
      desc: locale === "ka" ? "2024 წ. გაყიდვებში #1" : "#1 EV brand by sales in 2024",
    },
  ];
  ...
  return (
    <>
      ...
      <div className="bg-bg-secondary border-y border-white/[0.05]">
        <div className="section-container">
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-white/[0.06]">
            {stats.map((stat, i) => (
              <ScrollReveal key={stat.label} delay={i * 0.1} className="py-8 px-6 text-center">
                <div className="text-3xl md:text-4xl font-bold text-white mb-1" style={{ fontFamily: "var(--font-source-sans)" }}>
                  <AnimatedCounter value={stat.value} suffix={stat.suffix} />
                </div>
                <p className="text-xs text-white/40 uppercase tracking-widest" style={{ fontFamily: "var(--font-source-sans)" }}>
                  {stat.label}
                </p>
              </ScrollReveal>
            ))}
      ...
      <section className="py-section-sm md:py-section-lg">
        <div className="section-container">
          <ScrollReveal className="text-center mb-14">
            <p className="text-xs text-white/40 uppercase tracking-[0.2em] mb-4" style={{ fontFamily: "var(--font-source-sans)" }}>
              {locale === "ka" ? "ჩვენი ავტომობილები" : "Our Lineup"}
            </p>
            <h2 className="text-3xl md:text-display font-bold text-white mb-4" style={{ fontFamily: "var(--font-source-sans)", letterSpacing: "-0.02em" }}>
              {t("featuredTitle")}
            </h2>
            <p className="text-white/50 max-w-xl mx-auto font-light" style={{ fontFamily: "var(--font-source-sans)" }}>
              {t("featuredSubtitle")}
            </p>
          </ScrollReveal>
        ...
        <div className="relative section-container">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <ScrollReveal direction="left">
              <p className="text-xs text-accent uppercase tracking-[0.2em] mb-4" style={{ fontFamily: "var(--font-source-sans)" }}>
                {locale === "ka" ? "ჩვენ შესახებ" : "About GT Group"}
              </p>
              <h2 className="text-3xl md:text-display font-bold text-white mb-6" style={{ fontFamily: "var(--font-source-sans)", letterSpacing: "-0.02em" }}>
                {t("brandTitle")}
              </h2>
              <p className="text-white/55 text-lg leading-relaxed mb-8 font-light" style={{ fontFamily: "var(--font-source-sans)" }}>
                {t("brandText")}
              </p>
              <Link
                href="/contact"
              ...
              {features.map((item, i) => (
                <ScrollReveal key={item.title} delay={i * 0.1}>
                  <div className="p-5 rounded-card bg-white/[0.04] border border-white/[0.08] hover:border-white/[0.18] transition-all duration-300">
                    ...
                    <h3 className="text-sm font-semibold text-white mb-1" style={{ fontFamily: "var(--font-source-sans)" }}>{item.title}</h3>
                    <p className="text-xs text-white/45 font-light leading-relaxed" style={{ fontFamily: "var(--font-source-sans)" }}>{item.desc}</p>
                  </div>
                </ScrollReveal>
              ))}
            </div>
          ...
          {[0, 1].map((arrIdx) => (
            <div key={arrIdx} className="flex items-center gap-12 px-12">
              {["Sealion 06 DM-i", "Yuan Up DM-i", "Yuan Up EV", "Seal 06 DM-i"].map((name) => (
                <span key={name + arrIdx} className="flex items-center gap-4 text-xs text-white/30 uppercase tracking-widest" style={{ fontFamily: "var(--font-source-sans)" }}>
      ...
      <section className="py-24 text-center">
        <div className="section-container">
          <ScrollReveal>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4" style={{ fontFamily: "var(--font-source-sans)", letterSpacing: "-0.02em" }}>
              {locale === "ka" ? "მზადა ხართ ტესტ დრაივი?" : "Ready to experience the future?"}
            </h2>
            <p className="text-white/45 mb-10 font-light text-lg" style={{ fontFamily: "var(--font-source-sans)" }}>
              {locale === "ka" ? "დაჯავშნეთ ტესტ დრაივი — სრულიად უფასო" : "Book a free test drive — no commitment required"}
            </p>
