...
export default function ServicesHub({ initialSelection }: { initialSelection: Selection }) {
  ...
  const [finder, setFinder] = useState({ model: initialSelection.model ?? "", year: initialSelection.year ?? "", category: initialSelection.category ?? "", keyword: "" });
  const [searched, setSearched] = useState(Boolean(initialSelection.model && initialSelection.year && initialSelection.category));
  ...
  const filtered = useMemo(() => resultGroups.filter((id) => !finder.keyword || t(`results.${id}.title`).toLocaleLowerCase().includes(finder.keyword.toLocaleLowerCase())), [finder.keyword, t]);
  return <main className={styles.page}>
    ...
    <section id="product-finder" className={`${styles.section} ${styles.finder}`}><Picture desktop={finderDesktop} mobile={finderMobile} alt={t("images.finder")} /><div className={styles.finderShade} /><div className={`${styles.container} ${styles.finderInner}`}><div className={styles.finderCopy}><Heading t={t} section="finder" /></div><form className={styles.finderForm} onSubmit={(event) => { event.preventDefault(); setSearched(true); }}><Select t={t} label="model" value={finder.model} set={(model) => setFinder({ ...finder, model })} options={serviceModels.map((item) => [item.value, item.label])} /><Select t={t} label="year" value={finder.year} set={(year) => setFinder({ ...finder, year })} options={serviceYears.map((item) => [item, item])} /><Select t={t} label="category" value={finder.category} set={(category) => setFinder({ ...finder, category })} options={categories.map((key) => [key, t(`finder.categories.${key}`)])} /><label>{t("fields.keyword")}<input value={finder.keyword} onChange={(event) => setFinder({ ...finder, keyword: event.target.value })} /></label><button className={styles.primary}>{t("finder.primary")}</button><button type="button" className={styles.textButton} onClick={() => go("service")}>{t("finder.secondary")}</button></form></div><div className={`${styles.container} ${styles.results}`}>{searched ? <div className={styles.resultGrid}>{(filtered.length ? filtered : ["manual"]).map((key) => <article key={key}><span>{t(`results.${key}.category`)}</span><h3>{t(`results.${key}.title`)}</h3><p>{t(`results.${key}.body`)}</p><small>{t(`results.${key}.installation`)}</small><em>{t("finder.compatibility")}</em><a href="mailto:info@byd.ge">{t("finder.request")}</a></article>)}</div> : <p className={styles.empty}>{t("finder.empty")}</p>}</div></section>
