...
import { getTrimsForVersion, TIME_SLOTS } from "@/lib/test-drive";
import { NAME_MAX_LENGTH, validateName, type NameValidationError } from "@/lib/validation";

type Selection = { model?: string; trim?: string; year?: string; categories?: string[] };
...
const productPickerOptions: ProductPickerOption[] = serviceModels.map((model) => ({
  id: model.value,
  name: model.name.replace(/\s+(?:DM-i|EV)$/i, ""),
  image: model.image,
  powertrain: model.value.endsWith("-ev") ? "EV" : "PHEV",
  variants: getTrimsForVersion(model.value).map((trim) => ({ id: trim.id, name: trim.label })),
}));
...
export default function ServicesHub({ initialSelection }: { initialSelection: Selection }) {
  ...
  const tProductPicker = useTranslations("landing.services.form");
  const tSelect = useTranslations("formControls.select");
  const [active, setActive] = useState("service");
  const [finder, setFinder] = useState({ model: initialSelection.model ?? "", trim: initialSelection.trim ?? "", year: initialSelection.year ?? "", categories: initialSelection.categories ?? [], keyword: "" });
  ...
  return <main className={styles.page}>
    ...
    <section id="service" className={styles.section}><div className={`${styles.container} ${styles.split}`}><Picture desktop={serviceDesktop} mobile={serviceMobile} alt={t("images.service")} /><div><Heading t={t} section="official" /><div className={styles.serviceList}>{["maintenance", "diagnostics", "repair", "fluids"].map((key, index) => <article key={key}><span>{`0${index + 1}`}</span><div><h3>{t(`official.items.${key}.title`)}</h3><p>{t(`official.items.${key}.body`)}</p></div></article>)}</div><RequestForm t={t} tProductPicker={tProductPicker} kind="appointment" /></div></div></section>
    <section id="spare-parts" className={`${styles.section} ${styles.darkSurface}`}><div className={`${styles.container} ${styles.split} ${styles.reverse}`}><Picture desktop={partsDesktop} mobile={partsMobile} alt={t("images.parts")} /><div><Heading t={t} section="parts" /><ul className={styles.tickList}>{["genuine", "compatibility", "vin", "installation", "availability"].map((key) => <li key={key}>{t(`parts.assurance.${key}`)}</li>)}</ul><div className={styles.chips}>{partCategories.map((key) => <span key={key}>{t(`parts.categories.${key}`)}</span>)}</div><RequestForm t={t} tProductPicker={tProductPicker} kind="parts" /></div></div></section>
    ...
    <section id="product-finder" className={`${styles.section} ${styles.finder}`}><Picture desktop={finderDesktop} mobile={finderMobile} alt={t("images.finder")} /><div className={styles.finderShade} /><div className={`${styles.container} ${styles.finderInner}`}><div className={styles.finderCopy}><Heading t={t} section="finder" /></div><form className={styles.finderForm} onSubmit={(event) => { event.preventDefault(); setSearched(true); }}><div className={styles.fieldGroup}><span>{t("fields.model")}</span><ProductPickerField value={finder.model} options={productPickerOptions} onChange={(model) => setFinder({ ...finder, model, trim: "" })} onClear={() => setFinder({ ...finder, model: "", trim: "" })} clearLabel={tProductPicker("clearModel")} placeholder={tProductPicker("modelPlaceholder")} appearance="services" groupedOptions groupedOptionsLabel={tProductPicker("trimLabel")} secondaryValue={finder.trim} onSecondaryChange={(trim) => setFinder({ ...finder, trim })} aria-label={t("fields.model")} /></div><Select t={t} label="year" value={finder.year} set={(year) => setFinder({ ...finder, year })} options={serviceYears.map((item) => [item, item])} /><label>{t("fields.category")}<CustomSelect multiple value={finder.categories} onChange={(selected) => setFinder({ ...finder, categories: selected })} placeholder={tSelect("multiplePlaceholder")} options={categories.map((key) => ({ value: key, label: t(`finder.categories.${key}`) }))} /></label><label>{t("fields.keyword")}<input value={finder.keyword} onChange={(event) => setFinder({ ...finder, keyword: event.target.value })} /></label><div className={styles.finderActions}><button className={`${styles.primary} ${styles.finderAction}`}>{t("finder.primary")}</button><button type="button" className={`${styles.textButton} ${styles.finderAction}`} onClick={() => go("service")}>{t("finder.secondary")}</button></div><button type="button" className={styles.clearButton} onClick={() => { setFinder({ model: "", trim: "", year: "", categories: [], keyword: "" }); setSearched(false); }}>{t("finder.clear")}</button></form></div><div className={`${styles.container} ${styles.results}`}>{searched ? <div className={styles.resultGrid}>{(visibleResults.length ? visibleResults : ["manual"]).map((key) => <article key={key}><span>{t(`results.${key}.category`)}</span><h3>{t(`results.${key}.title`)}</h3><p>{t(`results.${key}.body`)}</p><small>{t(`results.${key}.installation`)}</small><em>{t("finder.compatibility")}</em><a href="mailto:info@byd.ge">{t("finder.request")}</a></article>)}</div> : <p className={styles.empty}>{t("finder.empty")}</p>}</div></section>
...
function RequestForm({ t, tProductPicker, kind }: { t: ReturnType<typeof useTranslations>; tProductPicker: ReturnType<typeof useTranslations>; kind: "appointment" | "parts" }) {
  ...
  return <form className={styles.request} noValidate onSubmit={(event: FormEvent) => { event.preventDefault(); const nextNameError = validateName(values.name ?? ""); setNameError(nextNameError); if (nextNameError) return setInvalid(true); if (fields.filter((field) => !field.optional).some(missingValue)) return setInvalid(true); setSent(true); }}><h3>{t(`${kind}.formTitle`)}</h3><p>{t(`${kind}.formText`)}</p><div className={styles.requestFields}>{fields.map((field) => {
    ...
    if (field.key === "model") return <div key={field.key} className={`${styles.fieldGroup} ${fieldClass ?? ""}`}><span>{label}</span><ProductPickerField value={values.model ?? ""} options={productPickerOptions} onChange={(value) => { setValue("model", value); setValue("trim", ""); }} onClear={() => { setValue("model", ""); setValue("trim", ""); }} clearLabel={tProductPicker("clearModel")} placeholder={tProductPicker("modelPlaceholder")} appearance="services" groupedOptions groupedOptionsLabel={tProductPicker("trimLabel")} secondaryValue={values.trim ?? ""} onSecondaryChange={(value) => setValue("trim", value)} aria-label={label} /></div>;
