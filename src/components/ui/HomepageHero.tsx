...
function animateWindowScroll(
  ...
  const step = (now: number) => {
    ...
    document.documentElement.scrollTop = startY + distance * easedProgress;
...
function startShowroomScroll() {
  ...
  // Phase 1: fast scroll to the first model section
  const targetY = getScrollYForElement(firstModel);
  const distance = Math.abs(targetY - window.scrollY);
  const phase1Duration = Math.min(980, Math.max(420, 280 + distance * 0.21));
...
function beginCinematicScroll() {
  ...
  // One continuous slow scroll through all models — 5% faster
  const duration = Math.min(8740, Math.max(3705, distance * 1.976));
