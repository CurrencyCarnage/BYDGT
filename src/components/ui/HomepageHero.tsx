...
const SHOWROOM_SETTLE_MS = 140;
const SHOWROOM_SECTION_PAUSE_MS = 240;

let activeShowroomRaf = 0;
let activeShowroomTimeout: number | null = null;
let detachShowroomCancelListeners: (() => void) | null = null;
...
function stopShowroomScroll() {
  if (activeShowroomRaf) {
    cancelAnimationFrame(activeShowroomRaf);
    activeShowroomRaf = 0;
  }
  if (activeShowroomTimeout !== null) {
    clearTimeout(activeShowroomTimeout);
    activeShowroomTimeout = null;
  }
  if (detachShowroomCancelListeners) {
    detachShowroomCancelListeners();
    detachShowroomCancelListeners = null;
  }
}
...
function attachShowroomCancelListeners() {
  ...
  const cancel = () => stopShowroomScroll();
...
function animateWindowScroll(
  ...
  const step = (now: number) => {
    ...
    if (progress < 1) {
      activeShowroomRaf = requestAnimationFrame(step);
      return;
    }
...
function startShowroomScroll() {
  ...
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    window.scrollTo({
      top: getScrollYForElement(showcase),
      behavior: "smooth",
    });
    return;
  }

  attachShowroomCancelListeners();

  const targetY = getScrollYForElement(showcase);
  const distance = Math.abs(targetY - window.scrollY);
  const phase1Duration = Math.min(900, Math.max(380, 280 + distance * 0.22));

  animateWindowScroll(targetY, phase1Duration, easeOutCubic, () => {
    activeShowroomTimeout = window.setTimeout(beginCinematicScroll, SHOWROOM_SETTLE_MS);
  });
}

function beginCinematicScroll() {
  const sections = Array.from(
    document.querySelectorAll<HTMLElement>("[data-model-section]")
  );
  ...
  const targets = sections
    .map((section, index) => {
      const extraOffset = index === 0 ? 8 : 24;
      return Math.min(maxScrollY, getScrollYForElement(section, NAVBAR_OFFSET + extraOffset));
    })
    .filter((target, index, all) => index === 0 || Math.abs(target - all[index - 1]) > 16);
  ...
  const runNext = () => {
    ...
    const targetY = targets[currentIndex];
    const distance = Math.abs(targetY - window.scrollY);
    const duration = Math.min(2600, Math.max(1500, 950 + distance * 0.85));

    animateWindowScroll(targetY, duration, easeInOutSine, () => {
      currentIndex += 1;
      ...
      activeShowroomTimeout = window.setTimeout(runNext, SHOWROOM_SECTION_PAUSE_MS);
    });
  };
